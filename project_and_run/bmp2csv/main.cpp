...
int main(int argc, char** argv)
{
	...
	{
		sprintf(output, "%s%s%s.csv", drive, dir, "Initial_wave_data2");
		bmp.WriteCsv(output, 3, min, max);
	}
	if ( strcmp(argv[1], "-i3" ) == 0 )
	{
		sprintf(output, "%s%s%s.csv", drive, dir, "Initial_wave_data2");
		bmp.WriteCsv(output, 4, min, max);
	}
	if ( strcmp(argv[1], "-i4" ) == 0 )
	{
		sprintf(output, "%s%s%s.csv", drive, dir, "Initial_wave_data2");
		bmp.WriteCsv(output, 5, min, max);
	}
	...
	if ( strcmp(argv[1], "-red" ) == 0 || strcmp(argv[1], "-blue" ) == 0 || strcmp(argv[1], "-red_blue" ) == 0)
	{
			...
			if ( strcmp(argv[1], "-blue" ) == 0 || strcmp(argv[1], "-red_blue" ) == 0)
			{
				for ( int n = 0; n < 5; n++ )
				{
					for ( int i = 0; i < bmp.H(); i++ )
					{
						for ( int j = 0; j < bmp.W(); j++)
						{
							if ( bmp.cell(i,j).r == 255 && bmp.cell(i,j).g == 255 && bmp.cell(i,j).r == 255 )
							{
								int s = 0;
								int d = 0;
								for ( int k = 1; k <= 5; k++ )
		...
		if ( strcmp(argv[1], "-red" ) == 0 || strcmp(argv[1], "-red_blue" ) == 0)
		{
			for ( int i = 0; i < bmp.H(); i++ )
			{
				for ( int j = 0; j < bmp.W(); j++)
				{
					if ( bmp.cell(i,j).r && bmp.cell(i,j).r+xx < 255 ) bmp.cell(i,j).r += xx;
					else
					{
						nn++;
					}
		...
		if ( strcmp(argv[1], "-blue" ) == 0 || strcmp(argv[1], "-red_blue" ) == 0)
		{
			for ( int i = 0; i < bmp.H(); i++ )
			{
				for ( int j = 0; j < bmp.W(); j++)
				{
					if ( bmp.cell(i,j).b && bmp.cell(i,j).b+xx < 255 ) bmp.cell(i,j).b += xx;
					else
					{
						nn++;
					}
		...
		if ( nn ) printf("overflow value.[%d]\n", nn);

		//for ( int i = 0; i < bmp.H(); i++ )
		//{
		//	for ( int j = 0; j < bmp.W(); j++)
		//	{
		//		bmp.cell(i,j).b = 255-bmp.cell(i,j).b;
		//		if ( bmp.cell(i,j).b < 0 ) bmp.cell(i,j).b = 0;
		//		bmp.cell(i,j).g = 0;
		//		bmp.cell(i,j).r = 255-bmp.cell(i,j).r;
		//		if ( bmp.cell(i,j).r < 0 ) bmp.cell(i,j).r = 0;
		//	}
		//}
	...
	if ( strcmp(argv[1], "-red2" ) == 0 || strcmp(argv[1], "-blue2" ) == 0 || strcmp(argv[1], "-red_blue2" ) == 0)
	{
		for (int i = 0; i < bmp.H(); i++)
		{
			for (int j = 0; j < bmp.W(); j++)
		...
		if ( strcmp(argv[1], "-red2" ) == 0 )sprintf(output, "%s%s%s2.bmp", drive, dir, "red");
		if ( strcmp(argv[1], "-blue2" ) == 0 )sprintf(output, "%s%s%s2.bmp", drive, dir, "blue");
		if ( strcmp(argv[1], "-red_blue2" ) == 0 )sprintf(output, "%s%s%s2.bmp", drive, dir, "red_blue");
		bmp.Write(output);
