...
#define c1_1 (c012.c1._1)
#define c1_2 (c012.c1._2)
#define c1_3 (c012.c1._3)
#define c0_1 (c012.c0._1)
#define c0_2 (c012.c0._2)
#define c2_1 c012.c2

/* Subroutine */ int dc3d0_(c012_t& c012, real *alpha, real *x, real *y, real *z__, real *
...
/* ----- */
    ...
    pp3 = *pot3;
    pp4 = *pot4;
    ua0_(c012, &xx, &yy, &dd, &pp1, &pp2, &pp3, &pp4, dua);
/* ----- */
    for (i__ = 1; i__ <= 12; ++i__) {
	if (i__ < 10) {
	    u[i__ - 1] -= dua[i__ - 1];
...
/* ======================================= */
    ...
    dccon1_(c012, &xx, &yy, &dd);
    ua0_(c012, &xx, &yy, &dd, &pp1, &pp2, &pp3, &pp4, dua);
    ub0_(c012, &xx, &yy, &dd, &zz, &pp1, &pp2, &pp3, &pp4, dub);
    uc0_(c012, &xx, &yy, &dd, &zz, &pp1, &pp2, &pp3, &pp4, duc);
/* ----- */
    for (i__ = 1; i__ <= 12; ++i__) {
...
/* Subroutine */ int ua0_(c012_t& c012, doublereal *x, doublereal *y, doublereal *d__, 
	doublereal *pot1, doublereal *pot2, doublereal *pot3, doublereal *
	pot4, doublereal *u) ACC_RESTRICTION
{
    /* Initialized data */
...
#else
	static_def doublereal pi2 = 6.283185307179586;
#endif
...
/* Subroutine */ int ub0_(c012_t& c012, doublereal *x, doublereal *y, doublereal *d__, 
	doublereal *z__, doublereal *pot1, doublereal *pot2, doublereal *pot3,
	 doublereal *pot4, doublereal *u) ACC_RESTRICTION
{
    /* Initialized data */
...
/* Subroutine */ int uc0_(c012_t& c012, doublereal *x, doublereal *y, doublereal *d__, 
	doublereal *z__, doublereal *pot1, doublereal *pot2, doublereal *pot3,
	 doublereal *pot4, doublereal *u) ACC_RESTRICTION
{
    /* Initialized data */
...
/* Subroutine */ int dc3d_(c012_t& c012, real *alpha, real *x, real *y, real *z__, real *
...
#else
	static_def doublereal eps = 1e-6;
#endif
...
/* ----- */
    ...
    xi[0] = *x - *al1;
    xi[1] = *x - *al2;
    if (ACC_MATH abs(xi[0]) < eps) {
	xi[0] = f0;
    }
    if (ACC_MATH abs(xi[1]) < eps) {
	xi[1] = f0;
    }
/* ====================================== */
/* =====  REAL-SOURCE CONTRIBUTION  ===== */
/* ====================================== */
    ...
    et[0] = p - *aw1;
    et[1] = p - *aw2;
    if (ACC_MATH abs(q) < eps) {
	q = f0;
    }
    if (ACC_MATH abs(et[0]) < eps) {
	et[0] = f0;
    }
    if (ACC_MATH abs(et[1]) < eps) {
	et[1] = f0;
    }
/* -------------------------------- */
...
/* ----- ON NEGATIVE EXTENSION OF FAULT EDGE */
    ...
    ket[0] = 0;
    ket[1] = 0;
    r12 = ACC_MATH sqrt(xi[0] * xi[0] + et[1] * et[1] + q * q);
    r21 = ACC_MATH sqrt(xi[1] * xi[1] + et[0] * et[0] + q * q);
    r22 = ACC_MATH sqrt(xi[1] * xi[1] + et[1] * et[1] + q * q);
...
/* ===== */
    for (k = 1; k <= 2; ++k) {
	for (j = 1; j <= 2; ++j) {
	    dccon2_(c012, &xi[j - 1], &et[k - 1], &q, &c0_2.sd, &c0_2.cd, &kxi[k - 
		    1], &ket[j - 1]);
	    ua_(c012, &xi[j - 1], &et[k - 1], &q, &dd1, &dd2, &dd3, dua);
/* ----- */
...
/* ===== */
	...
	for (j = 1; j <= 2; ++j) {
	    dccon2_(c012, &xi[j - 1], &et[k - 1], &q, &c0_2.sd, &c0_2.cd, &kxi[k - 
		    1], &ket[j - 1]);
	    ua_(c012, &xi[j - 1], &et[k - 1], &q, &dd1, &dd2, &dd3, dua);
	    ub_(c012, &xi[j - 1], &et[k - 1], &q, &dd1, &dd2, &dd3, dub);
	    uc_(c012, &xi[j - 1], &et[k - 1], &q, &zz, &dd1, &dd2, &dd3, duc);
/* ----- */
	    for (i__ = 1; i__ <= 10; i__ += 3) {
...
/* Subroutine */ int ua_(c012_t& c012, doublereal *xi, doublereal *et, doublereal *q, 
...
/* Subroutine */ int ub_(c012_t& c012, doublereal *xi, doublereal *et, doublereal *q,
	doublereal *disl1, doublereal *disl2, doublereal *disl3, doublereal *
	u) ACC_RESTRICTION
{
    /* Initialized data */
...
/* ----- */
	...
	if (*xi == f0) {
	    ...
	    x = ACC_MATH sqrt(c2_1.xi2 + c2_1.q2);
	    ai4 = f1 / c0_1.cdcd * (*xi / rd * c0_1.sdcd + f2 * ACC_MATH atan((*et * (
		    x + *q * c0_1.cd) + x * (c2_1.r__ + x) * c0_1.sd) / (*xi *
		     (c2_1.r__ + x) * c0_1.cd)));
	}
	ai3 = (c2_1.y * c0_1.cd / rd - c2_1.ale + c0_1.sd * ACC_MATH log(rd)) /
		c0_1.cdcd;
...
/* ----- */
    ...
    ai2 = ACC_MATH log(rd) + ai3 * c0_1.sd;
...
/* Subroutine */ int uc_(c012_t& c012, doublereal *xi, doublereal *et, doublereal *q,
	doublereal *z__, doublereal *disl1, doublereal *disl2, doublereal *
	disl3, doublereal *u) ACC_RESTRICTION
{
    /* Initialized data */
...
/* Subroutine */ int dccon0_(c012_t& c012, doublereal *alpha, doublereal *dip) ACC_RESTRICTION
{
    /* Initialized data */
...
/* ----- */
    ...
    c0_1.sd = ACC_MATH sin(*dip * p18);
    c0_1.cd = ACC_MATH cos(*dip * p18);
    if (ACC_MATH abs(c0_1.cd) < eps) {
	c0_1.cd = f0;
	if (c0_1.sd > f0) {
	    c0_1.sd = f1;
	}
...
/* Subroutine */ int dccon1_(c012_t& c012, doublereal *x, doublereal *y, doublereal *d__) ACC_RESTRICTION
{
    /* Initialized data */
...
/* ----- */
    if (ACC_MATH abs(*x) < eps) {
	*x = f0;
    }
    if (ACC_MATH abs(*y) < eps) {
	*y = f0;
    }
    if (ACC_MATH abs(*d__) < eps) {
	*d__ = f0;
    }
    ...
    c1_2.r__ = ACC_MATH sqrt(c1_2.r2);
    if (c1_2.r__ == f0) {
	return 0;
    }
...
/* Subroutine */ int dccon2_(c012_t& c012, doublereal *xi, doublereal *et, doublereal *q,
	doublereal *sd, doublereal *cd, integer *kxi, integer *ket) ACC_RESTRICTION
{
    /* Initialized data */
...
/* ----- */
    if (ACC_MATH abs(*xi) < eps) {
	*xi = f0;
    }
    if (ACC_MATH abs(*et) < eps) {
	*et = f0;
    }
    if (ACC_MATH abs(*q) < eps) {
	*q = f0;
    ...
    c2_1.r__ = ACC_MATH sqrt(c2_1.r2);
    if (c2_1.r__ == f0) {
	return 0;
    }
...
/* ----- */
	...
	c2_1.tt = ACC_MATH atan(*xi * *et / (*q * c2_1.r__));
    }
/* ----- */
    if (*kxi == 1) {
	c2_1.alx = -ACC_MATH log(c2_1.r__ - *xi);
	c2_1.x11 = f0;
	c2_1.x32 = f0;
    } else {
	rxi = c2_1.r__ + *xi;
	c2_1.alx = ACC_MATH log(rxi);
	c2_1.x11 = f1 / (c2_1.r__ * rxi);
	c2_1.x32 = (c2_1.r__ + rxi) * c2_1.x11 * c2_1.x11 / c2_1.r__;
    }
/* ----- */
    if (*ket == 1) {
	c2_1.ale = -ACC_MATH log(c2_1.r__ - *et);
	c2_1.y11 = f0;
	c2_1.y32 = f0;
    } else {
	ret = c2_1.r__ + *et;
	c2_1.ale = ACC_MATH log(ret);
